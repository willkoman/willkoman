...
| `controller_caps` | int (bitmask) | 3 | Capability bitmask Steam uses to filter compatible configs. |
...
| `bindings` | Map of slot name → binding string. Slot names depend on `mode` (e.g. `dpad_north`, `button_A`, `click`, `touch_menu_button_3`). |
| `settings` | Mode-specific settings (deadzones, haptic intensity, etc.). |
| `gameactions` | v3: SIAPI in-game action bindings. |
| `inputs` | v3: nested per-slot blocks containing `activators` (Full_Press / Long_Press / Double_Tap …). |
...
| `hotbar_menu` | Scrollable horizontal bar (up to 16 slots). |
...
Every edit mutates both the typed view and the raw AST. Serialisation walks the raw AST and emits text, so unknown fields stay intact.
